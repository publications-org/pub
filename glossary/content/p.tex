...
Here is a list of mathematical formulas, theorems, and definitions for our further work.\\
The reason for this is to have a basic tool-kit for the future problem we will tackle on.
...
\begin{definition}
	...
	\begin{align*}
		\frac{\phi(n^{s}!)}{\tau(n^{s}!)} + \frac{\phi(n^{s}!)}{\lambda(n^{s}!)} + \phi(n^{s}!) &= Z'; \quad x \in \mathbb{Z}.
	\end{align*}
...
\begin{definition}
	...
	\begin{align*}
		...
		\int \frac{\lambda(y) \sqrt{x}}{x} dx + \prod_{n=1}^{y}\rho(x) x^{n} &= \mathrm{C}'
	\end{align*}
	Admit results in a domain $\mathbb{D}^{+}$. Alternatively we can suppose the following equations.
	\begin{align*}
	...
	\end{align*}
	Assuming all variables are non-zero.
\end{definition}
...
\begin{definition}
	...
	\end{align*}
	And let define an equation of the previous result of:
	\begin{align*}
...
\begin{definition}
	...
	\begin{align*}
		B' &= 2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} - \phi;\\
...
\begin{definition}
	Let the following constant per be:
	\begin{align*}
		\mathrm{C}' &= \frac{\phi - 4}{2} - \phi;\\
...
\begin{definition}
	Let the following function of x:
	\begin{align*}
...
\begin{definition}
	Let the following definite integral:
	\begin{align*}
...
\bibliography{Bell_System_Hartley, S0022314X10001836, Fatou1906, A_Course_of_Pure_Mathematics, General_Theory_Dirichlet_Series, Riemann}
