...
\lettrine[lines=2, lhang=0.1, loversize=0.2]{T}he glossary's purpose is to provide a reference for theorems, definitions, and equations for the purpose of being reusable.\\
References are cited at the bottom of the glossary where credit is due.\\
There will be proofs part of another book, adjacent to this one. Where each steps will be explained.

%% ============================================================
\section{On Theorems and Definitions:}
%% ============================================================

Let ourselves define the following in rigorous mathematics in a manner that let us reuse them later by subtracting the functions with our own.\\
The theorems spans from Mathematics to Information Theory. 
...
\begin{theorem}
	Thus let the following based on [2.2]:
	\begin{align*}
...
\begin{theorem}
	Let for any real $\forall x \in \mathbb{R}.$ If and only If: $x \neq 0.$:
	\begin{align*}
		E(x) &= \frac{\gamma(x)}{\delta(x) + \lambda(x)}, \quad \gamma(x) \neq 0.
	\end{align*}
	Therefore, based on $E(x)$:
	\begin{align*}
	...
	\end{align*}
	Separately, let the followin $\forall x, n \in \mathbb{R}$:
	\begin{align*}
		\prod_{n=1}^{3}\rho(x) x^{n} + \int \frac{\gamma(x)}{\delta(x) + \lambda(x)} dx \in \mathbb{R}.
	\end{align*}
	Admits a solution in $\forall s = P' \in \mathbb{R},$ of the following:
	\begin{align*}
	...
	\begin{align*}
		...
		\Delta y &= \ln x,
	\end{align*}
	Separately, we find $\forall I \in \mathbb{R}$ that equals:
	\begin{align*}
...
\begin{definition}
	...
	\end{align*}
	And. For an equation of the previous result of [2.19]:
	\begin{align*}
...
\begin{theorem}
	...
	\end{align*}
	substituting $\frac{n - 1}{2}$ with $\phi$:
	\begin{align*}
...
\begin{theorem}
	...
	\begin{align*}
		...
		\sqrt{2}+2 + \frac{5}{2} &= x.
	\end{align*}
	And optionally expand the following for $x \in \mathbb{R}.$:
	\begin{align*}
...
\begin{definition}
	Let the DFT be:
	\begin{align*}
		X_k &= \sum_{m=0}^{N-1} x_{m}\mathrm{e}^{-i2\pi \frac{km}{N}}, \quad k = 0,\dots,N-1.
	\end{align*}
	For $x_{0}, \dots, x_{n-1}$ be complex numbers. Where $\mathrm{e}^{-i2\pi /n}$ is a primitive nth root of 1.
\end{definition}

\begin{theorem}
	Let the following identities for $\mathbb{R}.$:
	\begin{align*}
		\frac{ln(\pi^\mathrm{e})!}{\pi} &= (\mathrm{e} \log(\pi))!,\\
		\mathrm{H} &= \frac{ln(\pi^\mathrm{e})!}{\pi} + \frac{(\mathrm{e} \log(\pi))!}{\pi}, \quad \forall \mathrm{H} \in \mathbb{R}.
	\end{align*}
	From the following for $x = \frac{ln(\pi^\mathrm{e})!}{\pi}, \quad \forall x \in \mathbb{R}.$:
	\begin{align*}
	H &= \frac{ln(\pi^\mathrm{e})!}{\pi} + \frac{(\mathrm{e} \log(\pi))!}{\pi}.
	\end{align*}
...
\begin{definition}
	Defining the Chvátal–Sankoff constants for $\gamma_k \in \mathbb{R}.$:
	\begin{equation}
		\gamma_k = \lim_{n -> \infty} \frac{\mathrm{E}{\lambda_{n, k}}}{n}.
	\end{equation}
\end{definition}

\subsection{Final Remarks:}

\lettrine[lines=2, lhang=0.1, loversize=0.2]{T}he nine pages of theorems now comes to an end, stating them here was the first step to then develop a framework later in the 'AWB'. We will next plot and analyse our results based of our theorems and definitions.

\subsection{Moving forward:}
\lettrine[lines=2, lhang=0.1, loversize=0.2]{W}e will now proceed to plot and analyze the following theorems to analyze their convergence.

\section{Plotting and Analysis of the previous theorems:}
...
The following plot were written using 'python3' using 'matplotlib'.

\begin{remark}
	Per [2.31] for $x = \frac{ln(\pi^\mathrm{e})!}{\pi}, \quad \forall x \in \mathbb{R}.$ we have the following:
	\begin{equation}
