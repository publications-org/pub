\documentclass[10pt]{article}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsfonts}
\usepackage{textcomp}
\usepackage{amsthm}
\usepackage{mathtools}

\theoremstyle{plain}
\newtheorem{theorem}{Theorem}[section]
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{corollary}[theorem]{Corollary}
\newtheorem{proposition}[theorem]{Proposition}

\theoremstyle{definition}
\newtheorem{definition}[theorem]{Definition}
\newtheorem{example}[theorem]{Example}

\theoremstyle{remark}
\newtheorem{remark}[theorem]{Remark}
\title{%
	\vspace{-1.5em}%
	\large\textbf{Part Ten}%
}

\author{A. E. Mahrouss}
\date{\today}

\begin{document}
	
	\begin{definition}
		For the following result:
		\begin{align*}
			\sqrt{2}+2&=\sqrt{2} + x;\\
			2\sqrt{2}+2 &=x;\\
			2(\sqrt{2} + 1) &= x;
		\end{align*}
		Then we can solve for $l=x.$
		\begin{align*}
			l + r &\leq n - 1;\\
			2(\sqrt{2} + 1) + r &\leq n - 1.
		\end{align*}
		Also. There exists for $r \in \mathbb{D}^{+}.$
		\begin{align*}
			2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2}  &\leq \frac{n - 1}{2} + n - 1;\\
			2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} - \frac{n - 1}{2} &\leq n - 1.
		\end{align*}
	\end{definition}
	
	\begin{definition}
		Let the general definition per [1.1], be:
		\begin{align*}
			B' &= 2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} - \frac{n - 1}{2};\\
			&= \frac{\frac{\frac{n - 1}{2} - 4}{2} - \frac{n - 1}{2}}{2(\sqrt{2} + 1)}.
		\end{align*}
		Replacing $\frac{n - 1}{2}$ to solve for $\phi$:
		\begin{align*}
			B' &= 2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} - \phi;\\
			&= \frac{\frac{\phi - 4}{2} - \phi}{2(\sqrt{2} + 1)};\\
			C' &= \frac{\phi - 4}{2} - \phi.
		\end{align*}
	\end{definition}
	
	\begin{definition}
		Let the following constant per [1.2] be:
		\begin{align*}
			C' &= \frac{\phi - 4}{2} - \phi;\\
			C' &= \frac{\frac{1+\sqrt{5}}{2} - 4}{2} - \frac{1+\sqrt{5}}{2}.
		\end{align*}
	\end{definition}
	
	\begin{definition}
		Let the following:
		\begin{align*}
			2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2}  &\leq \frac{n - 1}{2} + n - 1;\\
			2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} - \frac{n - 1}{2} &\leq n - 1;\\
			2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} &\leq 0.
		\end{align*}
	\end{definition}
	
	\begin{definition}
		Let the following:
		\begin{align*}
			2(\sqrt{2} + 1) + \frac{\frac{n - 1}{2} - 4}{2} &= x;
		\end{align*}
		Solve for $n=5.$
		\begin{align*}
			2(\sqrt{2} + 1) + \frac{5}{2} &= x;\\
			\sqrt{2}+2 + \frac{5}{2} &= x.
		\end{align*}
		And optionally expanding:
		\begin{align*}
			2+\frac{5}{2} &= \frac{x}{\sqrt{2}};\\
			&= \frac{x}{\sqrt{2}}.
		\end{align*}
	\end{definition}
	
	\nocite{*}
	
	
\end{document}