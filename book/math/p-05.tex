...
\begin{align*}
		\tau(n^{s}!) + \lambda(n) &= \phi(n^{s}!) \in \mathbb{D}.		
\end{align*}
	Have the following equations:
	\begin{align*}
		\lambda(n) &= \frac{\phi(n^{s}!)}{\tau(n^{s}!)};\\
		\tau(n) &= \frac{\phi(n^{s}!)}{\lambda(n^{s}!)}.
	\end{align*}
	Let the following be defined:
	\begin{align*}
		\frac{\phi(n^{s}!)}{\tau(n^{s}!)} + \frac{\phi(n^{s}!)}{\lambda(n^{s}!)} + \phi(n^{s}!) &= Z'; \quad x \in \mathbb{R}^{+}.
	\end{align*}
\end{definition}
