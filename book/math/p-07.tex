...
\begin{definition}
	Let the following:
	\begin{align*}
		A' &= \gamma(x)+\Delta x + \frac{1}{\lambda(x)}; \quad \lambda(x) \neq 0.
	\end{align*}
\end{definition}
...
\begin{definition}
	Let the following:
	\begin{align*}
		\frac{x}{\lambda(x)} \gamma(x) &= \frac{\Delta x}{y} + Y';\\
		\frac{x}{\lambda(x)} \gamma(x) - \frac{\Delta x}{y} &= Y'; \quad \Delta x \in \mathbb{D}.
	\end{align*}
	For all valid domain $\mathbb{D}$.
\end{definition}
